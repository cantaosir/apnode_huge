...
<?php foreach ($config['servers'] as $i => $server) { ?>
  <div class="server">
  <h2><a target="_top" href="./?overview&s=<?php echo $i; ?>" ><?php echo isset($server['name']) ? $server['name'] : format_html($server['host'])?></a></h2>
  ...
  <tr><td><div>maxmemory:</div></td><td><div><?php echo format_size($info[$i]['Memory']['maxmemory'])?></div></td></tr>
